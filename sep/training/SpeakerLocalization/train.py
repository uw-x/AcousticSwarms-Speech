...
from .network import normalize_input, unnormalize_input
...
def test_epoch(model: nn.Module, device: torch.device,
    ...
    with torch.no_grad():
        for batch_idx, (data,
                        label_voice_signals,
                        window_condition) in enumerate(test_loader):
            data = data.to(device)
            ...
            output_signal = model(normed_data, window_condition)
            
            # Un-normalize
            output_signal = unnormalize_input(output_signal, means, stds)
            output_voices = output_signal

            loss = model.module.loss(output_voices, label_voice_signals)
            test_loss += loss.item()
...
def train_epoch(model: nn.Module, device: torch.device,
    ...
    for batch_idx, (data,
                    label_voice_signals,
                    window_condition) in enumerate(train_loader):
        data = data.to(device)
        label_voice_signals = label_voice_signals.to(device)
